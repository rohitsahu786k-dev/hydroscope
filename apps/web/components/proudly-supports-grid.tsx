...
export function ProudlySupportsGrid({ heading = "We proudly support" }: { heading?: string }) {
  return (
    <section aria-label={heading} className="bg-white py-9">
      <Container>
        <p className="mb-6 text-center text-[11px] font-extrabold uppercase tracking-[0.24em] text-hydro-blue2">
          {heading}
        </p>
        <ul className="grid grid-cols-7 gap-4 max-xl:grid-cols-4 max-md:grid-cols-3 max-sm:grid-cols-2">
          {proudlySupports.map((item) => (
            <li
              key={item.name}
              className="grid h-20 place-items-center rounded-lg border border-hydro-line bg-white p-4 transition hover:-translate-y-1 hover:border-hydro-blue2 hover:shadow-[0_20px_46px_-34px_rgba(9,36,76,0.7)]"
            >
              <Image
                src={item.image}
                alt={item.name}
